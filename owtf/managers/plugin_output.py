...
class POutputDB(BaseComponent, PluginOutputInterface):
    ...
    def plugin_output_exists(self, plugin_key, target_id):
        ...
        :rtype: `bool`
        """
        count = self.db.session.query(PluginOutput).filter_by(target_id=target_id, plugin_key=plugin_key).count()
        return (count > 0)
    ...
    def plugin_count_output(self):
        ...
        :rtype: `dict`
        """
        complete_count = self.db.session.query(PluginOutput).count()
        left_count = self.db.session.query(Work).count()
    ...
    def get_severity_freq(self, session_id=None):
        ...
        targets = []
        target_objs = self.db.session.query(Target.id).filter(Target.sessions.any(id=session_id)).all()
        for target_obj in target_objs:
            targets.append(target_obj.id)

        plugin_objs = self.db.session.query(PluginOutput).all()
